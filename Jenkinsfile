node{
  ...
  stage ('Build') {
	...
	stage ("running appscan on cloud"){
		appscan application: '13a06581-eb2c-4b1f-8002-6722126ae44e', credentials: 'ASOC_Staging', failBuild: true, failureConditions: [failure_condition(failureType: 'high', threshold: 4)], name: 'JPS_test', scanner: static_analyzer('${env.WORKSPACE}'), type: 'Static Analyzer', wait: true   
	}
